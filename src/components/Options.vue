<template>
  ...
  <div v-else class="grid w-11/12 grid-cols-1 mx-auto gap-y-8 lg:w-5/12">
      ...
      <div class="relative mt-1">
        ...
        <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
          ...
          <ListboxOptions
            ...
            <ListboxOption as="template" v-for="category in categories" :key="category.id" :value="category.name"
              ...
              <li :class="[
                ...
                <span v-if="selectedCategory" :class="[
    ...
    <Listbox as="div" v-model="selectedDifficulty">
      ...
      <div class="relative mt-1">
        ...
        <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
          ...
          <ListboxOptions
            class="absolute z-10 w-full py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-56 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            <ListboxOption as="template" v-for="difficulty in difficulties" :key="difficulty" :value="difficulty"
              ...
              <li :class="[
                ...
                <span v-if="selectedDifficulty" :class="[
                  active ? 'text-white' : 'text-purple-600',
                  'absolute inset-y-0 right-0 flex items-center pr-4',
                ]">
                  <CheckIcon class="w-5 h-5" aria-hidden="true" />
    ...
    <button class="w-full px-12 py-4 text-lg text-white transition bg-gray-600 rounded-lg hover:bg-gray-700"
      @click="startQuiz()">
      Start quiz
    </button>
  </div>
