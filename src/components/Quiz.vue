<template>
  ...
  <div v-else
    ...
    <div class="row-span-2">
      <div class="flex flex-col items-center justify-between min-h-full py-4 rounded-lg">
        <div class="flex my-4">
          <div v-for="(item, index) in store.data.results" :key="index"
            class="flex items-center justify-center w-3 h-3 mx-1 text-xs text-center text-white rounded" :class="{
        ...
        <div
          ...
          <h1 class="font-medium text-center md:text-lg" v-html="store.data.results[store.currentQuestion].question">
          </h1>
        </div>
      </div>
    </div>
